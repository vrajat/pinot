...
public class LogicalTableRouteProvider implements TableRouteProvider {
  ...
  public TableRouteInfo calculateRoutes(RoutingManager routingManager, BrokerRequest offlineBrokerRequest,
    ...
    List<PhysicalTableRouteInfo> offlineTableRoutes = new ArrayList<>();
    for (PhysicalTable physicalTable : _offlineTables) {
      PhysicalTableRouteInfo route =
          PhysicalTableRouteInfo.from(physicalTable.getTableName(), routingManager, offlineBrokerRequest, requestId);
      if (route != null) {
    ...
    List<PhysicalTableRouteInfo> realtimeTableRoutes = new ArrayList<>();
    for (PhysicalTable physicalTable : _realtimeTables) {
      PhysicalTableRouteInfo route =
          PhysicalTableRouteInfo.from(physicalTable.getTableName(), routingManager, realtimeBrokerRequest, requestId);
      if (route != null) {
